// ============================================================================
// BREAKOUT BOX VALVE CONTROLLER - FIRMWARE V1.4
// ============================================================================
...
//
// CHANGELOG V1.4:
// - CORRECTION CRITIQUE: Diviseur 12V 15k/3.3k (etait 10k/3.3k)
// - DIV_15V corrige: 5.545 (etait 4.03)
// - Hardware: BAT54S clamp + R serie 1k + 100nF filtre
// - Seuils ajustes pour marge alternateur (V12_MAX=15.5V)
//
// CHANGELOG V1.3:
...
// ADC Inputs (avec diviseurs CORRIGES sur Breakout Box)
#define PIN_ADC_12V     1   // +12V_PROT via div 15k/3.3k + BAT54S clamp
#define PIN_ADC_5V      2   // +5V_LOG via div 10k/10k
#define PIN_ADC_AUTO    3   // AUTO_REQ via div 10k/10k
#define PIN_ADC_OUT     4   // OUT_DRIVER via div 15k/3.3k + BAT54S clamp
#define PIN_ADC_CMD     5   // CMD via div 10k/10k
#define PIN_ADC_MANUAL  8   // MANUAL_REQ via div 10k/10k

// Stimulation Outputs (vers NPN open-collector sur Breakout Box)
#define PIN_STIM1 10  // -> Q1 base -> IN_A 74HC14 (simule pressostat)
#define PIN_STIM2 11  // -> Q2 base -> CLK CD4013 (simule bouton)

// ============================================================================
// DIVISEURS DE TENSION - CORRIGES V1.4
// ============================================================================
// 
// CANAUX 12V (12V_PROT, OUT_DRIVER):
//   R_HIGH = 15k, R_LOW = 3.3k
//   Ratio = (15 + 3.3) / 3.3 = 5.545
//   V_max mesurable = 3.0V * 5.545 = 16.6V (securite)
//   V_gpio @ 14.4V = 14.4 / 5.545 = 2.60V (nominal)
//   V_gpio @ 16.0V = 16.0 / 5.545 = 2.88V (alternateur max)
//   V_gpio @ 29.0V = 29.0 / 5.545 = 5.23V -> clampe 3.6V par BAT54S
//
// CANAUX 5V (5V_LOG, AUTO_REQ, CMD, MANUAL_REQ):
//   R_HIGH = 10k, R_LOW = 10k (inchange)
//   Ratio = 2.0
//   V_max mesurable = 3.0V * 2.0 = 6.0V
//
#define DIV_15V  5.545f  // 15k/3.3k pour canaux 12V [CORRIGE V1.4]
#define DIV_6V   2.00f   // 10k/10k pour canaux 5V

// Calibration factor (ajuste par routine calibration)
float g_calFactor = 1.000f;

// Seuils validation [AJUSTES V1.4]
#define V12_MIN  10.0f   // Cold-crank limite
#define V12_MAX  15.5f   // Alternateur charge max
#define V5_MIN   4.75f
#define V5_MAX   5.25f
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n=== BREAKOUT BOX V1.4 ===");
  Serial.println("Diviseur 12V: 15k/3.3k (ratio 5.545)");
  Serial.println("Protection: BAT54S clamp + R serie 1k");
  ...
  // Splash
  tft.setTextColor(TFT_CYAN, TFT_BLACK);
  tft.drawCentreString("BREAKOUT BOX", 86, 80, 2);
  tft.drawCentreString("V1.4", 86, 110, 4);
  tft.setTextColor(TFT_GREEN, TFT_BLACK);
  tft.drawCentreString("ISO 7637 Protected", 86, 160, 1);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawCentreString("VALVE Controller", 86, 190, 1);
  tft.drawCentreString("Mustang EcoBoost", 86, 210, 1);
