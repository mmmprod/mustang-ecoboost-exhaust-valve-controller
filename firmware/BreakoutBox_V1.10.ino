 ...
 * INTERFACE TECHNICIEN TACTILE
 * ESP32-C6 Touch LCD 1.9" (Waveshare)
 * https://www.waveshare.com/wiki/ESP32-C6-Touch-LCD-1.9
 * 
 * CHANGELOG V1.10:
 * - FIX #1: Race condition g_touch_flag (portDISABLE_INTERRUPTS)
 * - FIX #2: Touch desactive pendant runTest() (detach/attachInterrupt)
 ...
 * - Watchdog 5s
 * 
 * HARDWARE: ESP32-C6 Touch LCD 1.9" Waveshare
 * - MCU: ESP32-C6FH8 (8MB Flash, 160MHz)
 * - LCD: ST7789V2, 170x320, SPI
 * - Touch: CST816, I2C 0x15
 * - Extras: SD Card slot, QMI8658 IMU (non utilises)
 * 
 * PINOUT OFFICIEL WAVESHARE:
 * LCD (RESERVE):     GPIO 4,5,6,7,14,15
 * Touch I2C:         GPIO8 (SCL), GPIO18 (SDA)
 * Touch Control:     GPIO21 (RST-NC), GPIO22 (INT)
 * SD Card (LIBRE*):  GPIO19 (MISO), GPIO20 (CS)
 * IMU (LIBRE*):      GPIO1 (INT1), GPIO2 (INT2)
 * ADC disponibles:   GPIO 0,1,2,3
 * Digital libres:    GPIO 16,17,21,23 (+19,20 si pas SD)
 * 
 * (*) SD Card et IMU NON utilises dans ce projet
 *     GPIO19,20 reutilises pour BreakoutBox
 *     NE PAS INSERER DE SD CARD!
 * 
 * TFT_eSPI: Copier User_Setup_C6_Touch_LCD.h vers
...
// =============================================================================
// CONFIGURATION PINS V1.10 - ESP32-C6 Touch LCD 1.9" Waveshare
// =============================================================================
// GPIO RESERVES LCD (NE PAS TOUCHER): 4, 5, 6, 7, 14, 15
// GPIO TOUCH I2C (PARTAGE INA219):    8 (SCL), 18 (SDA)
// GPIO TOUCH CONTROL:                 21 (RST-NC), 22 (INT)
// 
// ATTENTION: GPIO19,20 sont aussi SD Card MISO/CS
//            NE PAS INSERER DE SD CARD si ces pins sont utilises!

// ADC (seuls GPIO ADC sur ESP32-C6: 0-6, dont 4-6 reserves LCD)
#define PIN_DIV_12V     0   // ADC - Diviseur 12V
#define PIN_DIV_5V      1   // ADC - Diviseur 5V (partage IMU_INT1, OK)
#define PIN_OUT_DIV     2   // ADC - Diviseur sortie (partage IMU_INT2, OK)
#define PIN_AUTO_REQ    3   // Digital IN - Signal pression

// Digital IO (reutilise GPIO SD Card car SD non utilisee)
#define PIN_CMD         19  // Digital IN - Commande active (SD_MISO)
#define PIN_STIM1       20  // Digital OUT - Stimulation pression (SD_CS)
#define PIN_STIM2       21  // Digital OUT - Toggle manuel (Touch RST NC)
#define PIN_MANUAL_REQ  23  // Digital IN - Requete manuelle (libre)
