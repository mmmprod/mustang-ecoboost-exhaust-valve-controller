/*
 * BreakoutBox_V1.7.ino
 * Diagnostic VALVE v4.9+
 * ESP32-C6-LCD-1.47
 * 
 * CHANGELOG V1.7 (Audit Validation):
 * - readVoltageOut(): Ajout validation raw<10 + saturation (coherence API)
 * - readCurrent(): Ajout isfinite() AVANT validation plage (protection NaN)
 * - analogSetAttenuation(): Macro conditionnelle Core 2.x/3.x
 * - modeLive(): Gestion erreurs vOut coherente avec v12/v5
 * 
 ...
 * 
 * HARDWARE REQUIS V1.7:
 * - R 220 Ohm serie sur GPIO18 (STIM1) cote BreakoutBox
 * - R 220 Ohm serie sur GPIO19 (STIM2) cote BreakoutBox
 * - Voir BreakoutBox_Circuit_V1_6.md pour details
 */
...
#define VOUT_FAULT_HIGH     15.0f     // V1.7: Tension max sortie driver (14.4V + marge)
...
void setup() {
    Serial.begin(115200);
    delay(100);
    Serial.println("BreakoutBox V1.7 - Init...");
    ...
    // --- ADC ---
    // V1.7: Compatibilite ESP32 Arduino Core 2.x et 3.x
    analogReadResolution(12);
    #if defined(ADC_ATTEN_DB_11)
        analogSetAttenuation(ADC_ATTEN_DB_11);  // Core 3.x
    #else
        analogSetAttenuation(ADC_11db);          // Core 2.x
    #endif
    ...
    // --- Charger calibration NVS avec validation ---
    ...
    Serial.println("Init complete - V1.7 Audit Validated");
}
...
// =============================================================================
// FONCTIONS LECTURE [MODIFIE V1.7]
// =============================================================================
float readVoltage12V() {
    int raw = analogRead(PIN_DIV_12V);
    
    // Detection capteur HS (ligne coupee ou GND)
    if (raw < V12_RAW_MIN) {
...
// V1.7: Ajout validation coherente avec readVoltage12V/5V
float readVoltageOut() {
    int raw = analogRead(PIN_OUT_DIV);
    
    // V1.7: Detection capteur HS (ligne coupee ou GND)
    if (raw < V12_RAW_MIN) {
        Serial.println("ADC_OUT FAULT: raw < 10 (capteur HS ou GND)");
        return ADC_ERR_SENSOR_HS;
    }
    
    float v_gpio = (raw / ADC_RESOLUTION) * ADC_VREF;
    float vOut = v_gpio * DIV_OUT_RATIO * g_calFactor;
    
    // V1.7: Detection saturation (sortie driver 0-14.4V max)
    if (vOut > VOUT_FAULT_HIGH) {
        Serial.printf("ADC_OUT SATURATED: %.1fV > 15V\n", vOut);
        return ADC_ERR_SATURATED;
    }
...
// V1.7: Ajout isfinite() AVANT validation plage
float readCurrent() {
    if (!g_ina219_ok) return 0.0f;
    
    float mA = ina219.getCurrent_mA();
    
    // V1.7: Protection NaN/Inf AVANT toute comparaison
    // NaN echappe aux comparaisons < et > (toujours false)
    if (!isfinite(mA)) {
        g_i2c_errors++;
        Serial.printf("INA219 FAULT: NaN/Inf detected [%d/%d]\n", 
...
// =============================================================================
// FONCTIONS STIMULATION
// =============================================================================
void stimAuto(bool state) {
    if (state) {
        g_stim1_start = millis();  // Timestamp pour timeout
    }
...
void modeLive() {
    ...
    tft.println("=== LIVE V1.7 ===");
    tft.println();
    
    // Affichage avec detection erreurs ADC
    ...
    // V1.7: Gestion coherente erreurs vOut
    if (vOut == ADC_ERR_SENSOR_HS) {
        tft.setTextColor(TFT_RED, TFT_BLACK);
        tft.println("OUT:  SENSOR?");
    } else if (vOut == ADC_ERR_SATURATED) {
        tft.setTextColor(TFT_RED, TFT_BLACK);
        tft.println("OUT:  SATURATED");
    } else {
        tft.setTextColor(TFT_WHITE, TFT_BLACK);
        tft.printf("OUT:  %.2fV\n", vOut);
    }
    ...
    // --- Serial ---
    // V1.7: Detection erreur incluant vOut
    if (v12 >= 0 && v5 >= 0 && vOut >= 0) {
...
void modeCalibration() {
    ...
    // Lecture brute avec annulation calibration
    int raw = analogRead(PIN_DIV_12V);
    float v_gpio = (raw / ADC_RESOLUTION) * ADC_VREF;
    ...
    // Validation resultat avant stockage
...
void handleSerialCommand(char cmd) {
    switch (cmd) {
        ...
        case 'R':
            g_minCurrent = 9999.0f;
            g_maxCurrent = -9999.0f;
            g_i2c_errors = 0;  // Reset compteur erreurs
        ...
        case 'I':
            Serial.println("=== INFO ===");
            Serial.printf("Firmware: V1.7 (Audit Validated)\n");
...
void printHelp() {
    Serial.println();
    Serial.println("=== BREAKOUT BOX V1.7 (Audit Validated) ===");
    ...
    Serial.println();
    Serial.println("Protections V1.7:");
    Serial.println("  - INA219: isfinite() + plage [-100, 1000] mA");
    Serial.println("  - ADC: fault detection 12V/5V/OUT");
    Serial.println("  - NVS: corruption recovery");
    Serial.println("  - STIM1: timeout 60s");
    Serial.println("  - GPIO: R220 serie requis (hardware)");
    Serial.println();
}
